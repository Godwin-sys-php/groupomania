  ...
  try {
    ...
    if (req.body.password && req.body.password.trim() !== "") {
      if (validator.isEmail(req.body.email) && schema.validate(req.body.password) && req.body.pseudo.length > 4 && req.body.name.length > 4) {
        User.verify(req.body.email, req.params.id)
          .then(results => {
            if (results.length > 0) {
              res.status(403).json({ existEmail: true });
            } else {
              User.verify(req.body.pseudo, req.params.id)
                .then(results => {
                  if (results.length > 0) {
      ...
      if (validator.isEmail(req.body.email) && req.body.pseudo.length > 4 && req.body.name.length > 4) {
        User.verify(req.body.email, req.params.id)
          .then(results => {
            if (results.length > 0) {
              res.status(403).json({ existEmail: true });
